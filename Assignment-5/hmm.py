...
def forward(pi, A, B, O):
  ...
  # Q3.1 Edit here
  ###################################################
  alpha = [[pi[j] * B[j][O[0]] for j in range(S)]]
  for k, o in enumerate(O[1:]):
    alpha = alpha + [[B[j][o] * sum([A[i][j] * alpha[k][i] for i in range(S)]) for j in range(S)]]
...
def backward(pi, A, B, O):
  ...
  beta = [[1, 1]]
  for k, o in enumerate(O[:-1]):
    beta = [[sum([beta[k][j] * A[i][j] * B[j][o] for j in range(S)]) for i in range(S)]] + beta
...
def seqprob_backward(beta, pi, B, O):
  ...
  # Q3.2 Edit here
  ###################################################
  prob = sum([beta[0][i] * pi[i] * B[i][O[0]] for i in range(len(pi))])
  return prob
...
def viterbi(pi, A, B, O):
  ...
  S = len(pi)
  N = len(O)
  delta = [[pi[j] * B[j][O[0]] for j in range(S)]]
  p = []
  for k, o in enumerate(O[1:]):
    di = [[delta[k][i] * A[i][j] * B[j][o] for i in range(S)] for j in range(S)]
    m = np.argmax(di, axis=1)
    p.append(list(m))
    delta = delta + [[di[i][m[i]] for i in range(len(m))]]

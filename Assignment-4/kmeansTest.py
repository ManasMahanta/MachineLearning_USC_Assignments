...
def transform_image(image, code_vectors):
    ...
    #print("code-vectors",code_vectors.shape)
    #print("image",image.shape)

    Rimage=image.reshape(image.shape[0]*image.shape[1],len(image.shape))

    C = np.array([np.argmin([np.dot(x_i-y_k, x_i-y_k) for y_k in code_vectors]) for x_i in Rimage])
    Assignment=C.reshape(image.shape[0],image.shape[1])

    for i in range(image.shape[0]):
        for j in range(image.shape[1]):
            for k in range(code_vectors.shape[0]):
                if(Assignment[i][j]==k):
                    NewImage[i][j]=code_vectors[k]

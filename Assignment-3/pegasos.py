...
def objective_function(X, y, w, lamb):
    ...
    N = X.shape[0]
    obj_value = (lamb / 2) * np.dot(w.T, w) + (1 / N) * sum([np.maximum(0, 1 - np.dot(yn, np.dot(w.T, xn))) for xn, yn in zip(X, y)])
...
def pegasos_train(Xtrain, ytrain, w, lamb, k, max_iterations):
    ...
    for iter in range(1, max_iterations + 1):
        ...
        # you need to fill in your solution here
        etat = 1 / (lamb * iter)
        wt = (1 - etat * lamb) * w + (etat / k) * sum([np.dot(ytrain[i], Xtrain[i]) for i in A_t]).reshape(D, 1)
        w = np.minimum(1, (1 / np.sqrt(lamb)) / np.dot(wt.T, wt)) * wt
...
def pegasos_test(Xtest, ytest, w, t = 0.):
    ...
    # you need to fill in your solution here
    N = len(ytest)
    pred = [np.dot(w.T, x) for x in Xtest]
    test_acc = sum([int(np.dot(ytest[i], pred[i])) >= 0 for i in range(N)]) / N
